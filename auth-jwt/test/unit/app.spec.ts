...
describe("Express App", () => {
  ...
  it("should respond with welcome message at GET /", async () => {
    const res = await request(app).get("/");
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ message: "Welcome to the Auth JWT API" });
  });

  it("should return 401 at GET /users when no API token is provided", async () => {
    const res = await request(app).get("/users");
    expect(res.status).toBe(401);
    expect(res.body).toEqual({ message: "API token not provided" });
  });

  it("should return 401 at GET /carts when no API token is provided", async () => {
    const res = await request(app).get("/carts");
    expect(res.status).toBe(401);
    expect(res.body).toEqual({ message: "API token not provided" });
  });

  it("should return 403 at GET /users with an invalid API token", async () => {
  ...
  it("should serve Swagger UI at GET /api-docs (without trailing slash)", async () => {
    const res = await request(app).get("/api-docs");
